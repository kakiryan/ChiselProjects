...
class upDownCounterUnrollUnitTests(counter: UpDownCounterUnroll) extends PeekPokeTester(counter) {
...
class UpDownCounterUnrollTester extends ChiselFlatSpec {
  behavior of "UpDownCounter"
  backends foreach {backend =>
    it should s"Count up or down depending on input $backend" in {
      Driver(() => new UpDownCounterUnroll, backend) ((counter) => new upDownCounterUnrollUnitTests(counter)) should be (true)
    }
  }
}
